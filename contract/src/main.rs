...
pub fn install_nft_contract() -> (ContractHash, ContractVersion) {
    let entry_points = {
...
pub extern "C" fn installChildContract() {
    ...
    // This value cannot be changed after installation.
    let collection_symbol: String = String::from("SYMBOL_PLACEHOLDER");

    // This represents the total number of NFTs that will
    // be minted by a specific instance of a contract.
    // This value cannot be changed after installation.
    let total_token_supply: u64 = 100; // for testing

    let allow_minting: bool = true;
    ...
    // This value cannot be changed after installation.
    let minting_mode: u8 = 1; // anyone can mint for testing
    ...
    // This value cannot be changed after installation.
    let ownership_mode: u8 = 2; // transferable token
    ...
    // This value cannot be changed after installation
    let whitelist_lock: u8 = 0; // unlocked, not used.
    ...
    // This value cannot be changed after installation.
    let nft_metadata_kind: u8 = 0;

    // The JSON schema representation of the NFT which will be minted.
    // This value cannot be changed after installation.
    let json_schema: String = runtime::get_named_arg("json_schema");
